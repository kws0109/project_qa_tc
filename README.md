...
장르·플랫폼을 가리지 않습니다 — 설명할 수 있는 게임이면 됩니다.
...
- **어떤 게임에든 붙습니다.** 도구가 아는 것은 특정 장르가 아니라 게임 일반의
  테스트 관점입니다 — 진입 경로 · 정상 경로 · 결과 검증 · 실패 경로 · 경계값 ·
  중단 · 미저장 이탈 · 재화 부족 · 미해금 접근 · 요소 표시 확인. RPG 든
  퍼즐이든 슈터든, 컨텐츠를 설명하면 해당하는 관점만 골라 TC 가 나오고
  해당 없는 관점은 이유와 함께 제외됩니다. 게임 추가는 yaml 파일 하나입니다.
...
어느 창을 찍을지는 게임 프로파일이 정하므로 창을 고를 필요가 없습니다.

![게임 화면 촬영 — 실행 중인 게임을 한 번에 찍어 첨부 스트립에 붙인다](docs/screenshots/capture-attach.png)
...
`profiles/` 에 들어 있는 네 개(`starrail` · `genshin` · `wuwa` ·
`bluearchive`)는 **예시**입니다 — 이 목록에 없는 게임도 위 파일만 만들면
똑같이 동작합니다. 창을 찍을 일이 없으면 `window` 없이 이름만 있어도 됩니다.
