...
ID가 그 코드를 가리키기 때문입니다. `slot init`을 유형 추가로 재실행할 때
`--code`를 생략하면 기존 코드가 그대로 유지됩니다. xlsx의 `테스트케이스`
시트는 제목·유형 칸 없이 `TC ID · 대분류 · 중분류 · 소분류 · 사전조건 · 절차
· 기대결과 · 우선순위 · 출처 · 근거 · 근거 상태` 열로 구성됩니다 — 중분류(화면)와
소분류(케이스 이름)를 함께 읽으면 어떤 테스트인지 알 수 있어야 하므로 별도
제목 칸이 없습니다.
