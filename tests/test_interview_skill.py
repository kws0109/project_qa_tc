...
def test_skill_step1_covers_the_ambiguous_content_message(cfg_env, capsys):
    ...
    위 테스트가 봉인하는 "신규 컨텐츠" 문구들과 달리 이 문구는 한동안 어느
    테스트도 잡고 있지 않았다. 최종 게이트에서 문구를 `여러 게임에 존재합니다`
    로 바꿔 봤더니 **스위트가 그대로 초록이었다** (F4). 이 분기가 무너지면
    모델은 게임이 둘인 상황에서 무엇을 해야 하는지 모른 채 멈춘다.

    (이 도크스트링은 원래 "위 테스트가 봉인한 세 문구와 달리 이 **네 번째**
    문구는" 이라고 적었는데, 위 테스트가 실측 문구 넷을 봉인하게 되면서 그
    번호가 다른 문구를 가리키게 됐다. 1단계 문구를 번호로 부르지 않는다.)
