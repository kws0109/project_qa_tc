...
INVISIBLE_VALUES = [
    ...
    ("\x1b", "C0 제어문자 ESC"),
    ("  \u200b \ufeff \t ", "보통 공백과 섞은 것"),
    # 이 하나만 성격이 다르다 — U+2028 은 `isspace()` 라 라운드 1a 의
    # `strip()` 가드도 **이미 잡았다**. 그러므로 아래 호출 지점 중
    # `test_knowledge_store.py` · `test_cli_slot.py` 의 "strip() 이 못 잡는다"
    # 설명은 이 값에는 해당하지 않는다 (실측: `is_blank` 를 `not text.strip()`
    # 로 되돌리면 61건이 깨지는데 `line-separator` 는 그 안에 없다). 그래도
    # 남기는 이유는 **가드가 통째로 사라진** 변이는 이 값도 놓치기 때문이다.
    ("\u2028", "줄 구분자 Zl (strip() 도 지운다)"),
